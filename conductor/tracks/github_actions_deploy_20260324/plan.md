...
1. [ ] **Add type-checking to `npm test`:** In `package.json`, change `"test": "vitest run"` → `"test": "tsc -b && vitest run"`.
...
4. [ ] **Verify `npm test` passes cleanly** (type-check + all tests green).
5. [ ] **Verify `npm run build` also passes** (defense in depth).
...
2. [ ] **Simplify deploy script:** In `package.json`, replace both `predeploy` and `deploy` with a single `deploy:manual` script: `"deploy:manual": "npm run build && gh-pages -d dist"`. Delete `predeploy` entirely — npm's `pre<script>` auto-hook doesn't fire for colon-namespaced scripts, so keeping it as a separate entry would be dead weight.
3. [ ] **Verify `package-lock.json` exists** — required by `npm ci` in the workflow.
4. [ ] Task: Manual verification
    - [ ] Check `dist/` folder (from Phase 0 build) contains `index.html`, `sw.js`, and `manifest.webmanifest`

### Phase 2: Create Workflow Definition
1. [ ] **Create Directory Structure:** Create `.github/workflows/` directory.
2. [ ] **Define Deploy Workflow:** Create `.github/workflows/deploy.yml` with:
    ...
    - **Job 1: `build`** on `ubuntu-latest`:
        ...
        - [ ] Setup Node.js (`actions/setup-node@v4`) with `node-version: 22` and `npm` caching.
        - [ ] Configure Pages (`actions/configure-pages@v5`).
        - [ ] Install dependencies (`npm ci`).
        - [ ] Run tests (`npm test`).
        - [ ] Run build (`npm run build`).
        - [ ] Verify PWA assets exist (`test -f dist/sw.js && test -f dist/manifest.webmanifest`).
        - [ ] Upload pages artifact (`actions/upload-pages-artifact@v4`, path: `./dist`).
...
3. [ ] Task: Manual verification
    - [ ] Validate YAML syntax (e.g., paste into https://www.yamllint.com/ or use a local linter)
    - [ ] Review the file to confirm trigger, permissions, and steps match the spec
