...
- *Full Card Drag and Drop*: implemented by the `task_interaction_refinement`
  conductor track; whole-card drag with no handle is current behavior (SPEC-007).
- *Swipe to Delete*: implemented by the same track (SPEC-009).
- *Notification Bell Placement*: the `NotificationBell` component no longer exists;
...
- *Centralized Project Specs Directory*: realized by this very system —
