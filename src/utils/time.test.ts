...
describe('calculateArrivalTime', () => {
  ...
  it('calculates arrival time when a task is in progress', () => {
    ...
    expect(result.toISOString()).toBe(
      new Date('2026-01-01T10:25:00Z').toISOString(),
    );
  });
  ...
  it('handles overtime consistently between both functions', () => {
    ...
    expect(etas.get('2')?.toISOString()).toBe(
      new Date('2026-01-01T10:20:00Z').toISOString(),
    );
    expect(arrivalTime.toISOString()).toBe(etas.get('2')?.toISOString());
  });
});
...
describe('calculateIntermediateETAs', () => {
  ...
  it('active + pending', () => {
    ...
    expect(etas.get('1')?.toISOString()).toBe(
      new Date('2026-01-01T10:05:00Z').toISOString(),
    );
    expect(etas.get('2')?.toISOString()).toBe(
      new Date('2026-01-01T10:25:00Z').toISOString(),
    );
  ...
  it('completed + active + pending', () => {
    ...
    expect(etas.get('1')?.toISOString()).toBe(
      new Date('2026-01-01T09:50:00Z').toISOString(),
    );
    expect(etas.get('2')?.toISOString()).toBe(
      new Date('2026-01-01T10:10:00Z').toISOString(),
    );
    expect(etas.get('3')?.toISOString()).toBe(
      new Date('2026-01-01T10:25:00Z').toISOString(),
    );
  });
