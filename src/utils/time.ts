...
export const calculateIntermediateETAs = (
  ...
  for (const task of tasks) {
    if (task.status === 'COMPLETED') {
      ...
      if (targetEndTime) {
        // Running: the due moment is a stored constant — anchor the ETA to it
        // instead of re-deriving it from a moving clock, so it cannot wobble
        // between ticks (TK-036). In overtime the target is in the past and
        // never projects there: everything starts from now.
        currentAccumulatedTime = Math.max(
